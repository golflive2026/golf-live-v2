...
export default function Settlement({ game, players, scores }: Props) {
  const entries = computeLeaderboard(players, scores);
  const settlement = computeSettlement(entries, scores, players, game);
  ...
  const even = settlement.filter(s => s.grandTotal === 0);

  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-2">
        <Receipt className="w-5 h-5 text-primary" />
        <h2 className="text-base font-bold">Final Settlement</h2>
      </div>
      ...
      {settlement.map(s => (
        <Card
          ...
          className={`border-border ${
            s.grandTotal > 0 ? "border-l-2 border-l-green-500" :
            s.grandTotal < 0 ? "border-l-2 border-l-red-500" : ""
          }`}
          data-testid={`card-settlement-${s.playerId}`}
        >
          <CardContent className="p-4">
            ...
            <div className="grid grid-cols-4 gap-2 text-center">
              <div>
                ...
                <MoneyDisplay amount={s.matchPlay} />
              </div>
              <div>
                <div className="text-[10px] text-muted-foreground uppercase">Birdies</div>
                <MoneyDisplay amount={s.birdies} />
              </div>
              <div>
                <div className="text-[10px] text-muted-foreground uppercase">Eagles</div>
                <MoneyDisplay amount={s.eagles} />
              </div>
              <div>
                <div className="text-[10px] text-muted-foreground uppercase">Special</div>
      ...
      <div className="text-xs text-muted-foreground text-center py-4">
        Positive = amount to collect · Negative = amount to pay
      </div>

      {/* Simple payout instructions */}
      {winners.length > 0 && losers.length > 0 && (
        <Card className="border-border">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Who Pays Whom</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            {(() => {
              ...
              while (pi < payers.length && ri < payees.length) {
                ...
                if (amount > 0) {
                  transactions.push({
                    from: payers[pi].playerName,
                    to: payees[ri].playerName,
                    amount,
                  });
                }
                payers[pi].remaining -= amount;
                payees[ri].remaining -= amount;
                if (payers[pi].remaining < 0.5) pi++;
                if (payees[ri].remaining < 0.5) ri++;
              }
